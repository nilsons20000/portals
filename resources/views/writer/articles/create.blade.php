...
<div class="container">
  ...
  @if(session()->has('message'))
   ...
   <div class="create-button" style="margin-top: 30px">
    <div class="alert alert-success success-send-article">
          {{ session()->get('message') }}
          <a class="btn btn-block link-home" href="{{URL::to('/writer')}}">Personīgais kabinets</a>
        </div>
    </div>
  ...
  <form class="form-horizontal" action="{{route('writer.article.store')}}" method="post" enctype="multipart/form-data">
    {{ csrf_field() }}
    {{-- Form include --}}
