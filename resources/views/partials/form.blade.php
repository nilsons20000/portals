<div class="group">
  <div class="first-group_element">
    ...
    @if (isset($article) && isset($article->id))
        <option value="2" {{ ($article->published == 2) ? 'selected' : '' }}>Piedāvāt</option>
        <option value="4" {{ ($article->published == 4) ? 'selected' : '' }}>Melnraksts</option>
        <option value="1" {{ ($article->published == 1) ? 'selected' : '' }}>Publicēta</option>
        <option value="3" {{ ($article->published == 3) ? 'selected' : '' }}>Atcelta</option>
    @else
        <option value="2">Piedāvāt</option>
        <option value="4">Melnraksts</option>
    @endif
    </select>
  </div>
...
<div class="rtq">
  <label for="parent_id">Vecāku kategorija</label>
